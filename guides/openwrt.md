...
9. We now extract the download image with multiple tools. First of all we untar the image: `tar -xvf FRITZ.Box_7490*.image ./var/tmp/filesystem.image --strip-components=3` then we will unsquash that with: `freetz-devel/tools/unsquashfs4-avm-be filesystem.image -e filesystem_core.squashfs` then move the extracted file with: `mv squashfs-root/filesystem_core.squashfs .` delete the empty directory: `rm -rf squashfs-root` and extract the fs again: `freetz-devel/tools/unsquashfs4-avm-be filesystem_core.squashfs -e lib/modules/dsp_vr9/`. You can now find the modem-firmware in `squashfs-root/lib/modules/dsp_vr9`. We only need to patch it with: `cd squashfs-root/lib/modules/dsp_vr9/` then: `bspatch vr9-B-dsl.bin vr9-A-dsl.bin vr9-A-dsl.bin.bsdiff` and `bspatch vr9-B-dsl.bin release-vr9-B-dsl.bin release-vr9-B-dsl.bin.bsdiff`. Congratulations, the hard part is done.
